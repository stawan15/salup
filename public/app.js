...
async function requestSummary(work, blocker, next) { const response = await fetch('/api/summarize', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({work, blocker, next}) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'OpenAI API ยังไม่พร้อมใช้งาน'); if (!data.summary) throw new Error('OpenAI ไม่ส่งผลลัพธ์กลับมา'); return data.summary; }
