...
async function requestSummary(work, blocker, next, voice, format) { const response = await fetch('/api/summarize', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({work, blocker, next, voice, format}) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'Gemini API ยังไม่พร้อมใช้งาน'); if (!data.summary) throw new Error('Gemini ไม่ส่งผลลัพธ์กลับมา'); return data.summary; }
...
$('summarizeBtn').addEventListener('click', async () => { const work=$('workInput').value.trim(), blocker=$('blockerInput').value.trim(), next=$('nextInput').value.trim(), voice=$('voiceMode').value, format=$('outputMode').value; if(!work){showToast('กรุณาใส่งานที่ทำวันนี้ก่อน'); $('workInput').focus(); return;} const btn=$('summarizeBtn'); btn.disabled=true; btn.textContent='กำลังสรุป...'; let summary; try { summary=await requestSummary(work,blocker,next,voice,format); } catch (error) { btn.disabled=false; btn.innerHTML='สรุปงาน <span>→</span>'; showToast(`AI ใช้งานไม่ได้: ${error.message}`); return; } $('summaryBox').innerHTML=summary; const plainSummary=summary.replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim(); const createdAt=new Date().toISOString(); const entries=getEntries(); entries.unshift({createdAt,title:`สรุปการทำงาน · ${thaiDate.format(today)}`,plainSummary,summary}); saveEntries(entries.slice(0,30)); const { error }=await saveToSupabase({work,blocker,next,summary}); renderStats(); $('savedTime').textContent=`บันทึกเมื่อ ${new Date().toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'})}`; btn.disabled=false; btn.innerHTML='สรุปงาน <span>→</span>'; showToast(error ? `บันทึกไม่สำเร็จ: ${error.message}` : 'สรุปด้วย AI และบันทึกเรียบร้อยแล้ว'); });
